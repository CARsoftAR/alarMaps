...
class _MapScreenState extends ConsumerState<MapScreen>
  ...
  void _checkPermissionsOnStartup() async {
    ...
    if (!basicStatus.isGranted) {
      if (mounted) {
        _showPermissionGuide(1, () async {
          ...
          if (granted)
            _checkPermissionsOnStartup(); // Re-check para el siguiente paso (Background)
        });
      }
      return;
    }
    ...
    final hasBackground = await _locationService
      ...
      if (mounted) {
        showDialog(
          ...
          builder: (context) => AlertDialog(
            ...
            content: const Text(
              'Para que la alarma funcione correctamente con la pantalla apagada, necesitamos que configures la app "Sin restricciones" en el uso de batería.',
            ),
            actions: [
              TextButton(
  ...
  void _getCurrentLocation() async {
    ...
    try {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text("Origen fijado en tu ubicación actual"),
  ...
  void _updateDistanceOffline(LatLng current) {
    ...
    if (dest != null) {
      ...
      if (ref.read(isSimulatingProvider) &&
          distance <= ref.read(selectedRadiusProvider)) {
            ...
            options: MapOptions(
              ...
              onTap: (tapPosition, point) async {
                if (!isActive && !isSimulating) {
                  ...
                  ref.read(currentDestinationAddressProvider.notifier).state =
                      address;
                ...
                markers: [
                  ...
                  if (userPos != null || origin != null)
                    Marker(
                      ...
                      child: Icon(
                        Icons.my_location,
                        color: (origin != null && origin != userPos)
                            ? Colors.green
                            : Colors.blue,
                        size: 40,
                      ),
                        ...
                        child: Column(
                          children: [
                            if (showOriginField) ...[
                              TextField(
                                ...
                                onSubmitted: (val) async {
                                  final results =
                                      await SearchService.performHardSearch(
                                        val,
                                      );
                                  if (results.isNotEmpty) {
                                    final res = results.first;
                                    final gLocation = LatLng(
                                      res.location.latitude,
                                      res.location.longitude,
                                    );
                                    ref
                                            .read(
                                              selectedOriginProvider.notifier,
                                            )
                                            .state =
                                        gLocation;
                            ...
                            TextField(
                              ...
                              onSubmitted: (val) async {
                                final results =
                                    await SearchService.performHardSearch(val);
                                if (results.isNotEmpty) {
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          return Padding(
            ...
            child: ActionChip(
              avatar: const Icon(Icons.place, size: 16, color: Colors.blue),
              label: Text(
                fav.name,
                style: const TextStyle(
                  fontSize: 12,
  ...
  void _selectFavorite(WidgetRef ref, FavoriteLocation favorite) {
    ...
    final currentPos =
        ref.read(userLocationProvider) ?? ref.read(selectedOriginProvider);
    if (currentPos != null) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(
          'Cargado: ${favorite.name} (${favorite.alarmRadius.toInt()}m)',
        ),
              ...
              SizedBox(
                ...
                child: ScaleTransition(
                  ...
                  child: ElevatedButton(
                        ...
                        : () async {
                            if (isActive) {
                              const platform = MethodChannel(
                                'com.example.alarmap/geofencing',
                              );
                              await platform.invokeMethod('removeGeofence');
                              await platform.invokeMethod('stopAlarmService');
                              ref.read(isAlarmActiveProvider.notifier).state =
                                  false;
                              WakelockPlus.disable();
                              _clearAlarmFromPrefs();
                            } else {
                              final basicGranted = await _locationService
                                  .checkPermissions();
                              if (!basicGranted) {
                                if (mounted) {
                                  _showPermissionGuide(1, () async {
                    ...
                    child: Text(
                      isActive ? "DETENER" : "ACTIVAR ALARMA",
  ...
  Future<void> _activateAlarm() async {
    ...
    if (destination != null) {
      ...
      try {
        ...
        debugPrint("Error registering geofence natively: $e");
      }
  ...
  void _selectSearchResult(SearchResult res) {
    ...
    final currentPos =
        ref.read(userLocationProvider) ?? ref.read(selectedOriginProvider);
    if (currentPos != null) {
      ref.read(currentDistanceProvider.notifier).state = _calculateHaversine(
        currentPos,
        gLocation,
      );
    }
    ...
    debugPrint(
      '📍 [Search] Posición sincronizada: ${gLocation.latitude}, ${gLocation.longitude}',
    );
  }
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Column(
          ...
          children: [
            Text(
              'Para que la alarma suene con la pantalla apagada, Android requiere el permiso de ubicación "Permitir todo el tiempo".',
            ),
            SizedBox(height: 15),
            Text(
              '1. Toca en CONFIGURAR PERMISOS.\n2. Ve a "Permisos" > "Ubicación".\n3. Selecciona "Permitir todo el tiempo".',
                  ...
                  return ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return ListTile(
                        ...
                        trailing: IconButton(
                          icon: Icon(
                            isSelected && _alarmService.isPlaying
                                ? Icons.stop
                                : Icons.play_arrow,
                            color: Colors.blue,
                          ),
                          onPressed: () async {
                            ...
                            if (!isPro && !isDefault) {
                              ProDialog.show(
                                ...
                                message:
                                    'La opción de cambiar el sonido de la alarma es exclusiva de ALARMap PRO. ¡Desbloquéala ahora!',
                              );
                              return;
                            }
                        ...
                        onTap: () {
                          ...
                          ref
                              .read(selectedAlarmProvider.notifier)
                              .setAlarm(option);
                          Navigator.pop(context);
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(
                              content: Text('Alarma fijada: ${option.title}'),
                              duration: const Duration(seconds: 2),
                            ),
                          );
                        },
